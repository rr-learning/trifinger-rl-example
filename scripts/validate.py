...
Ideally run it inside the Apptainer image like this:
    ...
    apptainer run -e --bind=$(pwd) trifinger_rl.sif python3 validate.py
...
def load_config(package_path: pathlib.Path):
    ...
    try:
        config = config["trifinger_rl_experiment"]
    except KeyError:
        raise RuntimeError("trifinger.toml: Missing section 'trifinger_rl_experiment'")

    try:
        task = config["task"]
    except KeyError:
        raise RuntimeError("trifinger.toml: Missing value trifinger_rl_experiment.task")
    assert task in ["push", "lift"], "trifinger.toml: Invalid task '{}'".format(task)

    policy_key = "{}_policy".format(task)
    assert policy_key in config, f"trifinger.toml: Missing value trifinger_rl_experiment.{policy_key}"
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            policy_key = "{task}_policy".format(**config)
